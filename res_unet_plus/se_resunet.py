...
class Se_PPP_ResUNet(nn.Module):
    def __init__(self, n_channels, n_classes, deep_supervision=False, dropout=False, rate=0.1):
        super(Se_PPP_ResUNet, self).__init__()
        self.deep_supervision = deep_supervision
        self.inc = inconv(n_channels, 64)
...
if __name__ == '__main__':
    ...
    model = Se_P_ResUNet(3, 1, deep_supervision=True)
    # model = Shallow_SeResUNet(3, 3, deep_supervision=True).cuda()
    model.apply(weights_init)

    # x = torch.randn((1, 1, 480, 480))
    summary(model=model, input_size=(1, 3, 480, 480), col_names=("input_size", "output_size"))
